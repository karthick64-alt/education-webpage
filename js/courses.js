...
// Complete courses data (all courses 1-12)
...
// Store in global scope - merge with existing data if available (from home.js)
if (!window.coursesData || !Array.isArray(window.coursesData) || window.coursesData.length === 0) {
    window.coursesData = allCoursesData;
} else {
    // Merge courses, avoiding duplicates by ID
    const existingIds = new Set(window.coursesData.map(c => c.id));
    const newCourses = allCoursesData.filter(c => !existingIds.has(c.id));
    window.coursesData = [...window.coursesData, ...newCourses];
}
...
function renderCourses() {
    ...
    // Ensure filteredCourses is initialized
    ...
    if (coursesToShow.length === 0) {
        container.className = 'row g-4 align-items-stretch';
        container.innerHTML = `
            <div class="col-12">
                <div class="text-center py-5">
                    ...
                    <button class="btn btn-primary mt-3" onclick="resetFilters(); renderCourses(); updateCourseCount();">
                        Reset Filters
                    </button>
                </div>
            </div>
    ...
    if (currentView === 'grid') {
        ...
        container.className = 'row g-3 align-items-stretch';
